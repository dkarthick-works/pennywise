...
export function EventDetailPage() {
  ...
  return <div className="content fade-in events-page">
    ...
    {!event ? query.isPending ? <p>Loading event…</p> : <div role="alert"><p>{query.error instanceof EventsApiError && query.error.status === 404 ? "This event is no longer available." : "Could not load this event."}</p><button className="btn btn-soft" onClick={() => void query.refetch()}>Retry</button></div> : <>
      ...
      <section className="card card-pad"><h2 className="card-h">Line items</h2>{!event.items.length ? <p className="muted">No items yet. Edit this event to add estimates.</p> : <div className="event-detail-items"><div className="event-detail-header" aria-hidden="true"><span /><span>Expected cost</span><span>Actual incurred</span></div>{event.items.map(item => <article className="event-detail-item" key={item.id}><h3>{item.name}</h3><p className={item.expected_cost === null ? "event-detail-empty" : "num"}>{item.expected_cost === null ? "Not entered" : money2(item.expected_cost)}</p><p className={item.actual_cost === null ? "event-detail-empty" : "num"}>{item.actual_cost === null ? "Not entered" : money2(item.actual_cost)}</p></article>)}</div>}</section>
      <section className="card card-pad"><h2 className="card-h">Event settings</h2><p className="event-setting-row">Free-money suggestions <span className={`suggestion-state${event.suggestions_enabled ? " is-on" : ""}`}>{event.suggestions_enabled ? "Enabled" : "Disabled"}</span></p>{event.note && <p className="event-note">{event.note}</p>}
        <div className="event-actions">
          {event.status !== "in_progress" && <button className={event.status === "planned" ? "btn btn-primary" : "btn btn-soft"} disabled={pending || conflict || unavailable} onClick={() => void changeStatus("in_progress")}>Start event</button>}
          {event.status !== "completed" && <button className={event.status === "in_progress" ? "btn btn-primary" : "btn btn-soft"} disabled={pending || conflict || unavailable || !event.summary.can_complete} onClick={() => void changeStatus("completed")}>Complete event</button>}
