...
Reserves are separate from `transactions`. `reserves` stores user-owned names and
lifecycle state; `reserve_operations` records user intent; and signed
`reserve_entries` derive every balance (there is no mutable balance column).
`reserve_operation_transactions` is the future link for normal rows generated by
...
| POST   | `/api/reserve-operations/funded-expenses` | atomically withdraw from a reserve and create protected normal cash income plus cash expense |
